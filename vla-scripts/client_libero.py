...
import numpy as np
from univla_client import websocket_client_policy as _websocket_client_policy
# import polars as pl
import rich
import tqdm
import tyro
...
def main(args: Args) -> None:
    obs_fn = {
        ...
        EnvMode.SIFER: _random_observation_sifer
    }[args.env]
    ...
    for _ in tqdm.trange(args.num_steps, desc="Running policy"):
        ...
        result = policy.infer(obs_fn()) # result is a dict
        action_chunk = result.get("actions", [])  # extract action chunk from dict
        
        # ---------- debug prints ----------
        print(f"action_chunk: {action_chunk}")
        print(f"action_chunk type: {type(action_chunk)}")
        print(f"action_chunk[0] type: {type(action_chunk[0])}")
        if isinstance(action_chunk[0], np.ndarray):
            print(f"action_chunk[0] shape: {action_chunk[0].shape}")
        if isinstance(action_chunk, list):
            print(f"action_chunk length: {len(action_chunk)}")
        # ----------------------------------
...
def _random_observation_sifer() -> dict:
    return {
        "state": np.random.rand(8),
        "full_image": np.random.randint(256, size=(224, 224, 3), dtype=np.uint8),
